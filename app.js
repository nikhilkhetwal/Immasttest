/* ====================================================================
   IMMAST × RISE — Course Calendar SPA
   Screen flow: landing → quarters → courses → PDF (overlay)
   ==================================================================== */
...
$$('.nav-link').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    if (target === 'courses') {
      // First visit → default to Q1. Otherwise use whatever the user last picked.
...
/* -------------------- Inline back buttons (one per screen) -------------------- */
...
function renderCourses(q) {
  ...
  list.innerHTML = '';
  // Apply quarter theme via class instead of inline styles
...
/* -------------------- PDF Viewer (real PDFs via iframe) -------------------- */
const overlay = $('#pdf-overlay');
const pdfFrame = $('#pdf-frame');
const pdfName = $('#pdf-name');
const pdfOpenNew = $('#pdf-open-new');
const closeBtn = $('#pdf-close');

function openPdf(q, c) {
  const src = pdfPathFor(c.name);
  pdfName.textContent = c.name;
  if (src) {
    // Hide PDF reader chrome — renders the page like an image in Chromium-based browsers
    const viewerArgs = '#toolbar=0&navpanes=0&scrollbar=0&statusbar=0&messages=0&view=FitH';
    pdfFrame.src = src + viewerArgs;
    pdfOpenNew.href = src;
    pdfOpenNew.classList.remove('hidden');
  } else {
    pdfFrame.src = 'about:blank';
    pdfOpenNew.classList.add('hidden');
  }
...
overlay.addEventListener('click', (e) => {
  // Close if the click landed anywhere outside the iframe, controls, or title label
  if (e.target.closest('#pdf-frame, .pdf-btn, .pdf-open-new, .pdf-title')) return;
  closePdf();
});
document.addEventListener('keydown', (e) => {
  if (overlay.classList.contains('open') && e.key === 'Escape') closePdf();
});
